...
import { redact, type RedactCategory, type ReplacementStyle } from "@webrex/redact";
...
export default function RedactionPanel() {
  ...
  const result = useMemo(() => redact(text, { enabledCategories: enabled, replacement: style }), [text, enabled, style]);
  ...
  const highlighted = (() => {
    ...
    let cursor = 0;
    let outIdx = 0;
    // Walk through redacted output and locate replacements (we re-derive by re-running)
    // Simpler approach: build from matches with stitched original slices
    let lastEnd = 0;
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-5">
            ...
            <div className="flex flex-wrap gap-2">
              {result.summary.map((s) => {
                ...
                return (
                  <button
                    key={s.category}
                    onClick={() => setEnabled((p) => ({ ...p, [s.category]: !p[s.category] }))}
          ...
          <div className="flex items-center gap-3 text-sm" style={{ color: "var(--color-text)" }}>
            <label htmlFor="rep-style" className="text-xs font-medium uppercase tracking-wider" style={{ color: "var(--color-text-muted)" }}>
              Replace with
            </label>
            <select
              id="rep-style"
