...
export default function VerifyCard({ tier, prompt, promptHtml, labEndpoint, expectedRegex, onConfidence }: Props) {
  ...
  const checkSoft = () => {
    ...
    setFeedback(value.length > 20 ? "🟡 Accepted with mid confidence." : "🟠 Accepted with low confidence — paste more for stronger verification.");
    emitConfidence(conf);
  };
    ...
    >
      <header className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono" style={{ color: "var(--color-text-faint)" }}>③</span>
          <h2 className="text-lg font-semibold" style={{ color: "var(--color-text)" }}>
            Verify
          </h2>
          <span className="text-xs px-2 py-0.5 rounded border" style={{ borderColor: "var(--color-border)", color: "var(--color-text-muted)" }}>
      ...
      {promptHtml ? (
        ...
        <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>{prompt}</p>
      ) : null}
        ...
        <>
          ...
          {showAiPanel && (
            ...
            >
              ...
              {aiLoading && <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>⏳ Asking AI…</p>}
              {aiResult === "ok" && (
              ...
              {aiResult === "warn" && (
                <div className="flex flex-col gap-1">
                  <span className="text-sm" style={{ color: "var(--color-warning)" }}>⚠️ AI is uncertain</span>
                  {aiFeedback && (
                    <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>{aiFeedback}</p>
                  )}
                </div>
              )}
